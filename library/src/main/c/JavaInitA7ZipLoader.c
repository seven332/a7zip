...
static jlong A7ZipLoader_NativeLoadLibrary(
    ...
    if (initialize == NULL) {
        dlclose(handle);
        THROW_EXCEPTION(env, "Can't find Initialize function in dynamic library %s: %s",
                        c_libname_copy, dlerror());
        return 0;
    }
    ...
    if (terminate == NULL) {
        dlclose(handle);
        THROW_EXCEPTION(env, "Can't find Terminate function in dynamic library %s: %s ",
                        c_libname_copy, dlerror());
        return 0;
    }
...
jint JNI_OnLoad(
    ...
    jint result = (*env)->RegisterNatives(env, clazz, a7zip_methods,
                                          sizeof(a7zip_methods) / sizeof(JNINativeMethod));
    if (result < 0) {
        return JNI_ERR;
    }
