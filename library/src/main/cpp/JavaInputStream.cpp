...
HRESULT JavaSequentialInStream::Initialize(JNIEnv *env) {
    ...
    class_native_sequential_in_stream = static_cast<jclass>(env->NewGlobalRef(
            class_native_sequential_in_stream));
    if (class_native_sequential_in_stream == nullptr) return E_OUTOFMEMORY;

    constructor_native_sequential_in_stream = env->GetMethodID(class_native_sequential_in_stream,
                                                               "<init>", "(J)V");
...
HRESULT JavaSequentialInStream::RegisterMethods(JNIEnv *env) {
    ...
    jint result = env->RegisterNatives(class_native_sequential_in_stream, stream_methods,
                                       std::extent<decltype(stream_methods)>::value);
    if (result < 0) {
...
HRESULT
JavaSequentialInStream::NewInstance(JNIEnv *env, ISequentialInStream *stream, jobject *object) {
    *object = nullptr;
    ...
    *object = env->NewObject(class_native_sequential_in_stream,
                             constructor_native_sequential_in_stream, stream);
